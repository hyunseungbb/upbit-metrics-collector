...
class UpbitTickerAdapter:
    ...
    async def connect(self) -> None:
        """WebSocket 연결"""
        try:
            logger.info("Ticker WebSocket 연결 시도", url=self.WS_URL)
            ...
            logger.info("Ticker WebSocket 연결 성공")
        except Exception as e:
            error_str = str(e)
            self.reconnect_attempts += 1
            logger.error("Ticker WebSocket 연결 실패", error=error_str, error_type=type(e).__name__, attempts=self.reconnect_attempts)
    ...
    async def subscribe(self, symbols: List[str], callback: Callable[[Dict[str, Any]], None]) -> None:
        ...
        try:
            ...
            logger.info("Ticker 구독 메시지 전송", symbols=symbols)
            
            # 수신 태스크 시작
            if not self.receive_task or self.receive_task.done():
                self.receive_task = asyncio.create_task(self._receive_loop())
                logger.info("Ticker 수신 루프 시작")
            
        except Exception as e:
            logger.error("Ticker 구독 실패", error=str(e), symbols=symbols)
    ...
    async def _receive_loop(self) -> None:
        ...
        while self.connected:
            try:
                ...
                if data.get("type") == "ticker" and self.callback:
                    receive_count += 1
                    self.last_data_time = datetime.utcnow()
                    if receive_count % 10 == 0:  # 10개마다 로그
                        logger.debug("Ticker 데이터 수신 중", count=receive_count)
                    await self.callback(data)
                    
            except ConnectionClosed:
                if not self.reconnecting:
                    logger.warning("Ticker WebSocket 연결 끊김, 재연결 시도")
                    self.connected = False
                    ...
                    try:
                        ...
                        logger.info("Ticker WebSocket 재연결 성공")
                        self.reconnecting = False
                    except Exception as e:
                        error_str = str(e)
                        logger.error("Ticker WebSocket 재연결 실패", error=error_str, attempts=self.reconnect_attempts)
                ...
                logger.warning("Ticker JSON 파싱 오류", error=str(e))
                continue
            except Exception as e:
                logger.error("Ticker 수신 루프 오류", error=str(e), error_type=type(e).__name__)
                await asyncio.sleep(1)
        
        logger.info("Ticker 수신 루프 종료", total_received=receive_count)
    ...
    def is_subscribed(self) -> bool:
        ...
        # 마지막 데이터 수신 시간이 90초 이내면 구독 중으로 간주 (ticker는 30~60초마다 저장)
        if self.last_data_time:
            time_since_last = (datetime.utcnow() - self.last_data_time).total_seconds()
            if time_since_last > 90:
                logger.warning("Ticker 구독 상태 의심", seconds_since_last=time_since_last)
                return False
    ...
    async def ensure_subscribed(self) -> bool:
        ...
        if not self.is_subscribed():
            logger.warning("Ticker 구독 상태 확인 실패, 재구독 시도")
            ...
            try:
                ...
                logger.error("Ticker 재구독 실패", error=str(e))
                return False
        return True
